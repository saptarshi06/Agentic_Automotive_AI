...
class GeminiClient:
    """
    Wraps AsyncLlamaStackClient for Google Gemini (via Llama Stack).
    Handles shield registration, inference (with tools), and safety checks.
    """
    ...
    def __init__(
        ...
        # Build provider data – Gemini adapter expects "gemini_api_key"
        self.provider_data = provider_data or {}
        if gemini_api_key:
            self.provider_data["gemini_api_key"] = gemini_api_key
    ...
    # Chat completion (supports tools)
    async def chat_completion(
        ...
        **kwargs: Any,
        ...
        """Call Gemini chat completions via Llama Stack."""
        extra = {}
        if tools:
            extra["tools"] = tools
        ...
        try:
            ...
            if response is None:
                raise GeminiClientError("Gemini returned no response.")
            return response
        except Exception as e:
    ...
    def _handle_inference_error(self, error: Exception) -> None:
        ...
        if "api key" in error_msg and ("invalid" in error_msg or "expired" in error_msg):
            logger.error("Gemini API key is invalid or expired.")
            raise GeminiClientError(
                "Gemini API key is invalid or expired. Check your GEMINI_API_KEY."
            ) from error
        if "quota" in error_msg or "rate limit" in error_msg:
            logger.error("Gemini rate limit or quota exceeded.")
            raise GeminiClientError(
                "Gemini rate limit or quota exceeded. Please try again later."
            ) from error
        raise GeminiClientError(f"Gemini inference error: {error}") from error

    # Safety / Shields (via Llama Stack)
    ...
    # Response parsing (Llama Stack response → LangChain AIMessage)
    @staticmethod
    def parse_chat_completion_to_aimessage(response: Any) -> AIMessage:
        """
        Convert a Llama Stack chat completion response into a LangChain AIMessage.
    ...
    # Utility: Convert LangChain tools to OpenAI‑compatible dicts
    @staticmethod
    def convert_langchain_tools_to_bedrock(tools: List[Any]) -> List[Dict[str, Any]]:
        """
        Convert LangChain BaseTool instances to the format expected by the
        Llama Stack (OpenAI‑style). The method name is kept for compatibility.
        """
        bedrock_tools = []
        for tool in tools:
